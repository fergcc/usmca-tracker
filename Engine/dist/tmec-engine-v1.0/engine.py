#!/usr/bin/env python3
"""TMEC Intelligence Engine — CLI orchestrator (standalone distribution).

Usage:
  python -m engine search --source /path/to/items.jsonl
  python -m engine enrich --source /path/to/items.jsonl --output /path/to/enriched.jsonl
  python -m engine run   --source /path/to/items.jsonl --output /path/to/enriched.jsonl
  python -m engine dry-run --source /path/to/items.jsonl
"""
...
def main() -> None:
    ...
    sp_search.add_argument("--source", required=True, help="Path to items.jsonl")
    ...
    sp_enrich.add_argument("--source", required=True, help="Path to items.jsonl")
    sp_enrich.add_argument("--output", required=True, help="Output path for enriched JSONL")
    ...
    sp_run.add_argument("--source", required=True, help="Path to items.jsonl")
    sp_run.add_argument("--output", required=True, help="Output path for enriched JSONL")
    ...
    sp_dry.add_argument("--source", required=True, help="Path to items.jsonl")
    sp_dry.add_argument("--output", default="/tmp/items_enriched.jsonl", help="Output path for enriched JSONL")
