...
class Enricher:
    ...
    def enrich_items(
        ...
        enriched = self.analyzer.analyze_batch(items_to_enrich)

        for i, orig in enumerate(items):
            if "aiSummary" not in orig and i < len(items_to_enrich):
                continue

        all_items = [it for it in items if "aiSummary" in it] + enriched

        if output_path:
            _save_jsonl(output_path, all_items)
